...
function salesCtrl($scope, $localStorage){
    $localStorage.savedSales = [];
    $localStorage.$default({savedSales: []});
    //console.log($localStorage);
    $scope.sales = $localStorage.savedSales;

    $scope.total = findTotal();

    var count = 0;

    $scope.addSale = function addSale(){
        //alert('button press');
        count = count +1;
        $scope.sales.push(new sale('sale' + count, 'A name', "this is a description", 10.10));
    }

/*
    // old function
    $scope.addSale = function (id, name, description, price) {
        count = count +1;
        $scope.sales.push(new sale(id + count, name, description, price));
        $localStorage.savedSales.push(new sale(id + count, name, description, price));
    }
*/
    $scope.addSale = function (name, costPrice, ProcurementFee, stampDuty){
        count = count +1;
        $scope.sales.push(new Sale(name, costPrice, ProcurementFee, stampDuty));

        // update the total
        $scope.total = findTotal();
    }

    $scope.showSalesMenu = function showSalesMenu(){
        if (angular.element('#salesMenu').width() < 1){
            angular.element('#salesMenu').width(315);
        } else {
            angular.element('#salesMenu').width(0);

        }
    }

    // sale class
    function sale (id, name, description, price){
        this.id = id;
        this.name = name;
    ...
    function findTotal(){
        ...
        for (var i = 0; i < $scope.sales.length; i ++){
            total += $scope.sales[i].subTotal;
        }
        return total;
    }

    $scope.deleteSale = function() {
        delete $localStorage.savedSales;
        $localStorage.savedSales = [];
        $scope.sales = $localStorage.savedSales;
    ...
    function Client (firstName, lastName, phoneNumber, email, address, comments){
    ...
    function Client (name, phoneNumber, email, address, comments){
        this.name = name;
        this.phoneNumber = phoneNumber;
        this.email = email;
        this.address = address;
    ...
    function Address(houseNo, street, suburb, state, postcode){
    ...
    function Sale(name, costPrice, ProcurementFee, stampDuty) {
        // wants to be edited
        if (!Date.now) {
            // for IE8 compatibility
            this.timeStamp = function () {
                var date = new Date()
                return new date.getDate() + '/' + (date.getMonth() + 1) + '/' + date.getYear() + '--' +
                    date.getHours() + ':' + date.getMinutes() + ':' + date.getSeconds();
            }
        } else {
            var date = new Date()
            this.timeStamp = date.getDate() + '/' + (date.getMonth() + 1) + '/' + date.getYear() + '--' +
                date.getHours() + ':' + date.getMinutes() + ':' + date.getSeconds();
        }
        ...
        this.subTotal = costPrice + ProcurementFee;
        this.stampDuty = stampDuty;

    }

    /*function Sale(name, vehicalType, costPrice, ProcurementFee, stampDuty) {
        // wants to be edited
        if (!Date.now) {
            // for IE8 compatibility
            this.timeStamp = function () {
...
function VechileCtrl($scope){
    $scope.avaliableList = [
        new Vechicle('Sadan', 'Toyota', 'Corola', '1998', 'Pink'),
        new Vechicle('Coupe', 'Hyndai', 'Coupe', '2004', 'Burgundy')
    ];
    $scope.selectedlist = [];

    function Vechicle(type, make, model, year, colour) {
        this.type =  type;
        this.make = make;
        this.model = model;
        this.year = year;
...
angular
    ...
    .controller('VechileCtrl', VechileCtrl);
